        ...
        */
        ...
        public static Boolean calcDamage(Obj_AI_Base targ)
        {
            ...
            if (_Player.ChampionName == "Aatrox")
            {
                ...
                if (checkForBuffAura) { if (HaveAatroxWDmg) { wdmg = _Player.CalculateDamageOnUnit(targ, DamageType.Physical, (float)(new[] { 60, 95, 130, 165, 200 }[Program.W.Level] + 1.0 * _Player.BaseAttackDamage)); } } else { wdmg = _Player.CalculateDamageOnUnit(targ, DamageType.Magical, (float)(new[] { 60, 105, 150, 195, 240 }[Program.W.Level] + 0.5 * _Player.TotalMagicalDamage)); }
            ...
            else if (_Player.ChampionName == "Amumu")
            {
                ...
                if (checkCDtoCalc) { if (Program.R.IsReady()) { rdmg = _Player.CalculateDamageOnUnit(targ, DamageType.Magical, (float)(new[] { 55, 110, 165, 220, 275 }[Program.R.Level] + 0.7 * _Player.TotalMagicalDamage)); } } else { rdmg = _Player.CalculateDamageOnUnit(targ, DamageType.Magical, (float)(new[] { 55, 110, 165, 220, 275 }[Program.R.Level] + 0.7 * _Player.TotalMagicalDamage)); }
            }
